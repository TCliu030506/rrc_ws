...
def ur2ros(ur_pose):
    ...
    np_q = tf.transformations.quaternion_from_matrix(np_T)
...
def ros2np(ros_pose):
    ...
    # orientation
    np_pose = tf.transformations.quaternion_matrix([ros_pose.orientation.x, ros_pose.orientation.y, \
...
def np2ros(np_pose):
    ...
    # ROS orientation 
    np_q = tf.transformations.quaternion_from_matrix(np_pose)
    ros_pose.orientation.x = np_q[0]
    ros_pose.orientation.y = np_q[1]
    ros_pose.orientation.z = np_q[2]
    ros_pose.orientation.w = np_q[3]
