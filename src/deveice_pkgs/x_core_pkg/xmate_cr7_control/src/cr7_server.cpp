    ...
    const std::shared_ptr<xmate_cr7_msg::srv::Cr7Script::Response> res)
{
    ...
    if(req->command == "stop"){
        ...
        int zone = req->zone;
        // 执行动作
        movel(target_pos, speed, zone);

    }
    else if(req->command == "open_rtControl_loop"){
        // 开启实时控制
        open_rtControl_loop(req->rt_type);

    }else if(req->command == "rtControl_jointpos_update"){
        ...
        // 更新目标位置
        rtControl_jointpos_update(joint_angles);
    }else if(req->command == "rtControl_cartesianpos_update"){
        std::array<double, 6> cartesian_pos;
        std::copy(req->pose.begin(), req->pose.begin() + std::min(req->pose.size(), cartesian_pos.size()), cartesian_pos.begin());
        // 更新目标位置
        rtControl_cartesianpos_update(cartesian_pos);
    }
    ...
    }else if(req->command == "calibrateForceSensor"){
        bool all_axes = req->all_axes;
        int axis_index = req->axis_index;
        bool success = calibrateForceSensor(all_axes, axis_index);
        res->result = success ? "Calibration successful" : "Calibration failed";

    }else if(req->command == "getEndTorque"){
        std::string ref_type_str = req->ref_type;
        ...
        res->torque.assign(joint_torque_measured.begin(), joint_torque_measured.end());
        res->external_torque.assign(external_torque_measured.begin(), external_torque_measured.end());
        res->cart_torque.assign(cart_torque.begin(), cart_torque.end());
        res->cart_force.assign(cart_force.begin(), cart_force.end());
...
void Cr7Server::stop()
{
    ...
    // 等待控制器真正停到 idle 再清空队列，避免“moving unsupported operation”
    for (int i = 0; i < 200; ++i) { // 最多等待约 2s
    ...
    robot.moveReset(ec);
    if (ec) {
        std::cerr << "Failed to stop and reset movement: " << ec.message() << std::endl;
        return;
    }
    // else{//测试用
    //     std::cout << "Successfully stopped and reset movement." << std::endl;
    // }
}
...
void Cr7Server::movel(std::array<double, 6> target_pos, int speed, int zone)
{
    CartesianPosition target_position(target_pos);
...
void Cr7Server::rtControl_jointpos_update(std::array<double, 6> joint_angles)
{
    // 更新目标位置
...
void Cr7Server::rtControl_cartesianpos_update(std::array<double, 6> cartesian_pos)
{
    // 更新目标位置
    target_pose = cartesian_pos;
    // 测试
    std::cout << "rtControl_cartesianpos_update" << std::endl;
...
bool Cr7Server::calibrateForceSensor(bool all_axes, int axis_index)
{
    bool success = false;
