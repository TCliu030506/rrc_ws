...
/**
 * @brief 主程序入口
 * 该函数用于连接机器人，根据机器人型号设置相关参数，并执行相应的点位跟随示例。
 * 
...
int main() {
  ...
  // 使用std命名空间中的所有成员
  using namespace std;
  // 使用rokae::RtSupportedFields命名空间中的所有成员
  ...
  // 定义错误码对象，用于存储可能出现的错误信息
  error_code ec;
  // 定义更新线程对象
  std::thread updater;
  // 定义xMateRobot对象，用于与机器人进行交互
  xMateRobot robot;

  try {
    // 尝试连接到机器人，使用指定的远程IP和本地IP
