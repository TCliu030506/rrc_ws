/*
* V2.3.4 更新时间：2023-6-7
API
1.优化USB和网口的的通讯。
2.增加工具坐标转换函数，可以把工具的坐标系转到注册板坐标系下：
...
*		无
*************************************************************************/
DLLExport void* Aim_API_Initial();//AimHandle &aimHandle
...
DLLExport int Aim_GetCountOfToolInfo(AimHandle aimHandle);
/************************************************************************/
/**
*	描述：
...
/**
*	描述：
*		对所选择的工具和注册板（标定板）进行操作，把工具的坐标系转到注册版坐标系下。在调用前，
*	    需要调用Aim_InitToolCoordinateRenewWithToolId函数设置所选择的注册板（标定板）和工具。
*		本函数（以及获取标记点坐标的函数）需多次调用，直到info.isCalibrateFinished为true
*	参数：
*		marker ：当前采集的标记点
*		ProInfo：返回削骨工具的参数，当标志位ProInfo.isCalibrateFinished为true时，结果有效
*	返回值：
*		AIMOOE_OK: 函数执行成功。
*		AIMOOE_ERROR: 函数执行失败。
*************************************************************************/
