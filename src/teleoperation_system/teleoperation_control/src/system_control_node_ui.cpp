...
public:
    ...
    // 输入内窥镜运动平台的末端位姿，返回O系下的机械臂位姿（[x,y,z,rx,ry,rz]）
    std::array<double, 6UL> trans_PFtoO_6(std::array<double, 6> pos)
    {
        // 求A坐标系下机械臂的末端位姿
        std::array<double, 6> P_A = pf_trans_inv(pos);
        // 求O坐标系下的机械臂位姿
        std::array<double, 6UL> P_O6 = trans_AO(P_A);
        return P_O6;
    }
...
public:
    TeleControlClient() : Node("tele_control_client"), robot("192.168.2.160", "192.168.2.101"),controller("/dev/ttyUSB0")
    {
        ...
        // 初始化订阅者，订阅名为 "ui_record_topic" 的话题 
        ui_record_subscriber = this->create_subscription<ui_control_msg::msg::UiControl>(
            "ui_record_topic", 10, std::bind(&TeleControlClient::ui_record_callback, this, std::placeholders::_1));
        ...
        file << "序号, x, y, z, rx, ry, rz, pf_x, pf_y, pf_z \n";
    ...
    ~TeleControlClient() {
        ...
        // 关闭文件
        if (file.is_open()) file.close(); // 显式关闭文件

    };

private:
    rclcpp::Publisher<xmate_cr7_msg::msg::Cr7State>::SharedPtr xmate_state_publisher;
    rclcpp::Publisher<xmate_cr7_msg::msg::Cr7State>::SharedPtr xmate_target_pos_publisher;
    ...
    rclcpp::Subscription<ui_control_msg::msg::UiControl>::SharedPtr ui_record_subscriber;
    ...
    int record_flag = 0; // 0: 不录制 1: 录制

    int record_count = 0; // 录制次数

    std::ofstream file; // 用于记录数据的文件流
    ...
    std::array<double, 3> pf_pos_mapping = {-0.96, 1.94, -39.59};   //手柄位置映射到PF控制模式末端位置

     std::array<double, 6UL> current_posture_cr7 = {0, 0, 0, 0, 0, 0}; // 实时位姿，XYZrxryrz表示   
    ...
    double pf_pos_default[3] = {-0.96, 1.94, -39.59};  //单位为mm
    ...
    void robot_initialize()
    {
        ...
        MoveJCommand startPosNew({0.125,-0.620,0.435,-M_PI,-42.0/180*M_PI,-90.0/180*M_PI});//260128 直线版本遥操作 初始位置
        robot.moveAppend({startPosNew}, id, ec);
        robot.moveStart(ec);
        waitRobot(robot, running);

        // 运动至PF的初始位置
        // controller.pf_movej(62,62,58,5);
        // 打印出测试字符
    ...
    void automatic_navigation()
    {
        ...
        // MoveJCommand puncture_pos({0.200,-0.600,0.360,160.0/180*M_PI,50.0/180*M_PI,70.0/180*M_PI});// 20260121 遥操作测试
        MoveJCommand middle_pos({0.199923,-0.809598,0.365000,151.439/180*M_PI,-34.598/180*M_PI,-69.026/180*M_PI});// 20260128 遥操作测试-过渡点
        MoveJCommand puncture_pos({0.203703,-0.804926,0.319435,151.439/180*M_PI,-34.598/180*M_PI,-69.026/180*M_PI});// 20260128 遥操作测试
        robot.moveAppend({middle_pos, puncture_pos}, id, ec);
        robot.moveStart(ec);
        waitRobot(robot, running);
        // 打印出测试字符
    ...
    void open_rtControl_loop()
    {
        ...
        current_posture = robot.posture(rokae::CoordinateType::endInRef, ec);
    ...
    {   
        ...
        else if(control_mode_falg == 2)
        {
            ...
            if(!pf_open_flag)
            {
                controller.pf_readp(pf_pos_default[0], pf_pos_default[1], pf_pos_default[2]);
                pf_open_flag = true;
                // 日志测试
                RCLCPP_INFO(this->get_logger(), "pf_pos_default: %.2f, %.2f, %.2f", pf_pos_default[0], pf_pos_default[1], pf_pos_default[2]);
            }
            // 模式切换衔接
            if(mode_change_flag == 1)
            {
            ...
            // 计算PF的目标位置
            pf_pos_mapping[0] = pf_pos_default[0] - sigema7_pos_mapping[0] * 1000 * mapping_ratio * 1.2;
            pf_pos_mapping[1] = pf_pos_default[1] - sigema7_pos_mapping[1] * 1000 * mapping_ratio * 1.2;
            ...
            // 日志测试
            RCLCPP_INFO(this->get_logger(), "pf_target: %.2f, %.2f, %.2f", pf_pos_mapping[0], pf_pos_mapping[1], pf_pos_mapping[2]);
        ...
        {
            // 日志 测试用
            RCLCPP_INFO(get_logger(), "记录当前从端位置开始");
            // 更新记录计数
            record_count++;
            // 记录当前从端位置
            // 计算当前机械臂的位置（x,y,z,rx,ry,rz）
            // Utils::transArrayToPosture(target_pose, current_posture_cr7);
            current_posture_cr7 =  trans.trans_PFtoO_6(xmate_pos_mapping);
            // 已知当前PF位置（pf_pos_mapping）
            
            // 写入已创建的CSV文件
            // 写入当前从端位置到CSV文件（序号,x,y,z,rx,ry,rz，theta1,theta2,theta3）
            file << record_count << "," << current_posture_cr7[0] << "," << current_posture_cr7[1] << "," << current_posture_cr7[2] << "," << current_posture_cr7[3] << "," << current_posture_cr7[4] << "," << current_posture_cr7[5] << "," << pf_pos_mapping[0] << "," << pf_pos_mapping[1] << "," << pf_pos_mapping[2] << "\n";
            file.flush(); // 立即刷新缓冲区
            // 录制完成后，将录制标志位设为0
            record_flag = 0;

            // 日志 测试用
            RCLCPP_INFO(get_logger(), "记录当前从端位置完成");
    ...
    {   
        ...
        if(control_flag == 2)
        {
            ...
            if(!open_flag)
            {
                ...
                // 初始化机械臂目标位置发布者
                xmate_target_pos_publisher = this->create_publisher<xmate_cr7_msg::msg::Cr7State>("xmate_target_pos", 10);
                // 启动机械臂状态发布线程
    ...
    void ui_record_callback(const ui_control_msg::msg::UiControl::SharedPtr msg)
    {   
        record_flag = msg->record_flag;
        std::cout << "record_flag: " << record_flag << std::endl;
        // 日志 测试用
        RCLCPP_INFO(get_logger(), "录制标志位: %d", record_flag);
    }
    ...
    void xmate_state_pub()
    {
        while (rclcpp::ok()) 
        {
            ...
            // 构建消息并发布目标位置
            auto target_msg = xmate_cr7_msg::msg::Cr7State();
            target_msg.header.stamp = this->now();
            std::array<double, 6UL> current_target_posture =  trans.trans_PFtoO_6(xmate_pos_mapping);
            target_msg.cr7_pos[0] = current_target_posture[0];
            target_msg.cr7_pos[1] = current_target_posture[1];
            target_msg.cr7_pos[2] = current_target_posture[2];
            target_msg.cr7_pos[3] = current_target_posture[3];
            target_msg.cr7_pos[4] = current_target_posture[4];
            target_msg.cr7_pos[5] = current_target_posture[5];
            xmate_target_pos_publisher->publish(target_msg);
            // 休眠一段时间
