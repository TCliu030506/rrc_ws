...
std::atomic<int> keyboard_input{0}; // 0 表示无输入
...
protected:
    void keyPressEvent(QKeyEvent *event) override {
        if (event->key() == Qt::Key_Escape) {
            ...
            keyboard_input.store('z', std::memory_order_relaxed);
        } else if (event->key() == Qt::Key_X) {
            keyboard_input.store('x', std::memory_order_relaxed);
        } else if (event->key() == Qt::Key_1) {
            keyboard_input.store('1', std::memory_order_relaxed);
        } else if (event->key() == Qt::Key_2) {
            keyboard_input.store('2', std::memory_order_relaxed);
        } else if (event->key() == Qt::Key_3) {
            keyboard_input.store('3', std::memory_order_relaxed);
        } else if (event->key() == Qt::Key_S) {
            keyboard_input.store('s', std::memory_order_relaxed);
        } else {
...
private:
    ...
    {
        while (rclcpp::ok()) {
            int key = keyboard_input.exchange(0, std::memory_order_relaxed);
            if (key == 'z') {
                auto msg = ui_control_msg::msg::UiControl();
