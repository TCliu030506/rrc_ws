...
private:
  ...
  rclcpp::Publisher<ur5_msg::msg::RobotState>::SharedPtr robotstate_pub;
  ...
  double joint_pos[6], joint_vel[6];    //save joint msg
  ...
  void fillRobotstateMsg()
  {
    ...
    Eigen::Matrix4d mat = ur5.tffwardkmatics(joint_pos);
    Eigen::Matrix3d mat_r = mat.block(0,0,3,3);
    Eigen::AngleAxisd axangaxis(mat_r);
    for (int i = 0; i < 3; i++)
    {
      robotstate_msg.carte_pos[i] = mat(i,3);
      robotstate_msg.carte_pos[i+3] = axangaxis.angle()*axangaxis.axis()(i);
    }
...
sensor_pipeline::sensor_pipeline(/* args */):Node("ur5_msg_demo")
{
  ...
  jointstate_sub = create_subscription<sensor_msgs::msg::JointState>("joint_states", 10,
    std::bind(&sensor_pipeline::jointstateCallback, this, std::placeholders::_1));

  
  robotstate_pub = this->create_publisher<ur5_msg::msg::RobotState>("robotstate", 10);//创建一个发布者对象用于发布 ur5_msg::msg::RobotState类型的消息到名为 "robotstate" 的话题上。


  robotstate_msg.header.frame_id = "ur5";
