...
from math import atan2, acos, asin, degrees, radians, pi
import numpy as np
from math import *
import math
import cmath
...
def ur2ros(ur_pose):
    ...
    # Ros orientation
    pos_euler = [ur_pose[3],ur_pose[4],ur_pose[5]]
    np_q = Quaternion.from_euler(pos_euler,axes = ['x', 'y', 'z'])
    ros_pose.orientation.x = np_q.x
    ros_pose.orientation.y = np_q.y
    ros_pose.orientation.z = np_q.z
    ros_pose.orientation.w = np_q.w
...
def ros2np(ros_pose):
    """Transform ROS Pose format to numpy quaternion format.

    Args:
        ros_pose: A ROS Pose object.

    Returns:
        A numpy quaternion array.
    """

    # Extract quaternion from ROS Pose object
    quat = [
...
def np2ros(np_pose):
    ...
    # ROS orientation 
    # 旋转矩阵转欧拉角
    np_qq = np.array([[np_pose[0,0], np_pose[0,1], np_pose[0,2]],
                      [np_pose[1,0], np_pose[1,1], np_pose[1,2]],
                      [np_pose[2,0], np_pose[2,1], np_pose[2,2]]])
    euler = rot2euler(np_qq)
    #欧拉角转四元数
...
def inv_kin(p, q_d, i_unit='r', o_unit='r'):
    ...
    # # Preprocessing
    # if isinstance(p, Pose):  # ROS Pose format
    #     T_06 = ros2np(p)
    # elif isinstance(p, list):  # UR format
    #     T_06 = ros2np(ur2ros(p))

    # if i_unit == 'd':
    #     q_d = [radians(i) for i in q_d]

    # # Ensure T_06 is a 4x4 transformation matrix
    # T_06 = np.array(T_06).reshape((4, 4))
    ...
    # Convert T_06 to NumPy array if it's not already
    T_06 = np.array(T_06)

    # Check if T_06 has the correct shape
    if T_06.shape != (4, 4):
        raise ValueError("T_06 should be a 4x4 matrix.")

    # Now reshape T_06 to (4, 4)
    T_06 = T_06.reshape((4, 4))
    ...
    # Constants
    d4 = 0.1  # Replace with your actual d4 value
    d6 = 0.1  # Replace with your actual d6 value
    a2 = 0.1  # Replace with your actual a2 value
    a3 = 0.1  # Replace with your actual a3 value

    # theta1
    P_05 = T_06[0:3, 3] - d6 * T_06[0:3, 2]
    phi1 = atan2(P_05[1], P_05[0])
    phi2 = acos(d4 / np.sqrt(P_05[0] ** 2 + P_05[1] ** 2))
    theta1 = [pi / 2 + phi1 + phi2, pi / 2 + phi1 - phi2]
    theta[0:4, 0] = theta1[0]
    theta[4:8, 0] = theta1[1]
    ...
    theta5 = []
    for i in range(2):
        theta5.append(acos((P_06[0] * np.sin(theta1[i]) - P_06[1] * np.cos(theta1[i]) - d4) / d6))
    for i in range(2):
        theta[2 * i, 4] = theta5[0]
        theta[2 * i + 1, 4] = -theta5[0]
        theta[2 * i + 4, 4] = theta5[1]
    ...
    for i in range(8):
        # theta3
        T_46 = np.dot(HTM(4, theta[i]), HTM(5, theta[i]))
        T_14 = np.dot(np.linalg.inv(HTM(0, theta[i])), T_06.dot(np.linalg.inv(T_46)))
        P_13 = T_14.dot(np.array([[0, -d4, 0, 1]]).T) - np.array([[0, 0, 0, 1]]).T
        if i in [0, 2, 4, 6]:
            theta[i, 2] = -cmath.acos((np.linalg.norm(P_13) ** 2 - a2 ** 2 - a3 ** 2) / (2 * a2 * a3)).real
            theta[i + 1, 2] = -theta[i, 2]
        # theta2
        theta[i, 1] = -atan2(P_13[1], -P_13[0]) + asin(a3 * np.sin(theta[i, 2]) / np.linalg.norm(P_13))
        # theta4
        T_13 = np.dot(HTM(1, theta[i]), HTM(2, theta[i]))
        T_34 = np.linalg.inv(T_13).dot(T_14)
        theta[i, 3] = atan2(T_34[1, 0], T_34[0, 0])
