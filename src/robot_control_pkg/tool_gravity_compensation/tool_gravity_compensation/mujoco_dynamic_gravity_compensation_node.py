...
class MujocoDynamicGravityCompensationNode(Node):
    def __init__(self):
        super().__init__('mujoco_dynamic_gravity_compensation_node')

        self.declare_parameter('wrench_in_topic', '/sensors/wrench')
        ...
        self.declare_parameter('link_frames', ['asm_tool_base_link', 'asm_tool_link1', 'asm_tool_link2'])
        self.declare_parameter('link_masses', [0.065358, 0.099171, 0.88511])
        self.declare_parameter('link_com_x', [3.1503e-15, -0.00057962, 0.0002913455])
        self.declare_parameter('link_com_y', [0.0, -0.00051221, 0.000318296])
        self.declare_parameter('link_com_z', [0.01886, -0.0055231, -0.00323911])
        ...
        self.sub = self.create_subscription(WrenchStamped, self.wrench_in_topic, self.wrench_cb, 10)
        ...
        self.get_logger().info(
            'MuJoCo dynamic gravity compensation ready. '
...
def main(args=None):
    ...
    node = MujocoDynamicGravityCompensationNode()
    try:
        rclpy.spin(node)
    finally:
        node.destroy_node()
