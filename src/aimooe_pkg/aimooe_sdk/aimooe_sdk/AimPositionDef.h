...
extern "C" {
	...
	*/
	...
	struct T_AIMPOS_DATAPARA
	{
		...
		T_Img_Info dualimg;
		T_Img_Info colorimg;
		std::array<char, 20> hardwareinfo;//型号信息
	...
	*/
	...
	struct T_AimToolDataResult
	{
		...
		std::array<float, 4> Qoxyz;
		std::array<std::array<float, 3>, 3> Rto;
		std::array<float, 3> Tto;

		std::array<float, 3> OriginCoor;
		std::array<float, 3> tooltip;
		std::array<float, 3> toolmid;
		std::array<float, 3> toolCstip;
		std::array<float, 3> toolCsmid;
	...
	struct T_AimToolDataResultSingle
	{
		...
		std::array<int, 16> toolPtId;	//工具上的点在系统的坐标点集下的索引，对于不匹配的点，其索引为-1，工具上的点不能超过16个，若超出，只显示前16个。
	...
	{	
		...
		float Dis;//左右工具原点的距离
		std::array<float,3> Angle;//左右工具法向量的夹角，单位为弧度
	};
	struct T_ToolFileData
	{
		...
		char tooType;
		int markerNumbers;
		std::array<float, 16 * 3> MarkerCoordinate;//若markerNumbers>16则时显示前16个标记点坐标；
		int  constTipNum;
