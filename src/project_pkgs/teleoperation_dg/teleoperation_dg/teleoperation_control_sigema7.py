...
class Subscriber(Node):
    ...
    def __init__(self):
        ...
        #     10
        # )
        self.subscription_sigema7 = self.create_subscription(
            Sigema7Position,
            '/sigema7_pos',
            self.handle_msg_sigema7,
            10
        )
        ...
        self._pos_scale = [1.0, 1.0, 1.0]      # X、Y、Z 比例
        # 例如：主端旋转 1 rad，对应从端旋转 0.2 rad
        self._rot_scale = [0.3, 0.3, 0.3]      # RX、RY、RZ 比例
    ...
    def handle_msg_sigema7(self, msg):
        # 存储接收到的数据，供发布线程使用
        with self._lock:
            # 这里用 deepcopy，避免另一个线程修改同一对象导致竞态
            self._master_pose = copy.deepcopy(msg.sigema7_pos)
            ry = self._master_pose[4]
            rz = self._master_pose[5]
            self._master_pose[4] = rz
            self._master_pose[5] = ry
            # 打印测试
            self.get_logger().info(
                f'Received Master Pose: x={self._master_pose[0]:.3f}, y={self._master_pose[1]:.3f}, z={self._master_pose[2]:.3f}, '
                f'rx={self._master_pose[3]:.3f}, ry={self._master_pose[4]:.3f}, rz={self._master_pose[5]:.3f}'
