...
int main(int argc, char** argv) {
    ...
    try
    {
        ...
        std::string csv_path = package_path + "/resource/03 轨迹跟随测试实验.csv";
    ...
    // 创建文件保存订阅数据
    std::string filename = "03 轨迹跟随测试实验 测量数据(3).csv";
    ...
    for(size_t i=0; i<points.size(); i++) {
        ...
        std::cout << "移动至点位 " << (i+1) << ": (" 
                  ...
                  << points[i][1] << ", " 
                  << points[i][2] << ")" << std::endl;
        controller.pf_movej(points[i][0], points[i][1], points[i][2], 8);
        sleep(1); // 等待0.5秒，确保移动完成
        ...
        rclcpp::Rate rate(10);
        auto start = node->now();
        while ((node->now() - start).seconds() < 1.0) {
