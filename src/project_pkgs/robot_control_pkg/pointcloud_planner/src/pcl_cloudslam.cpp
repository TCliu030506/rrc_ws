...
using pointcloudslam_cpp::extract_corner_candidates_xy;
using pointcloudslam_cpp::filter_by_normal_dot;
using pointcloudslam_cpp::filter_by_platform_height;
using pointcloudslam_cpp::fit_plane_ransac;
using pointcloudslam_cpp::keep_largest_cluster;
using pointcloudslam_cpp::orient_plane_toward_workpiece;
using pointcloudslam_cpp::PointCloudXYZ;
using pointcloudslam_cpp::PlaneFitResult;
using pointcloudslam_cpp::ConcavePathParams;
using pointcloudslam_cpp::ConcavePathResult;
using pointcloudslam_cpp::generate_concave_path;
using pointcloudslam_cpp::fit_workpiece_frame;
using pointcloudslam_cpp::transform_cloud_to_workpiece;
using pointcloudslam_cpp::WorkpieceFrameParams;
using pointcloudslam_cpp::WorkpieceFrameResult;
...
bool save_matrix4d(const std::string &path, const Eigen::Matrix4d &matrix)
{
...
bool load_matrix4d(const std::string &path, Eigen::Matrix4d *matrix)
{
    if (matrix == nullptr) {
        return false;
    }
...
{
    if (!cloud || cloud->empty() || min_point == nullptr || max_point == nullptr) {
        return false;
    }

    *min_point = Eigen::Vector3d::Constant(std::numeric_limits<double>::max());
    *max_point = Eigen::Vector3d::Constant(std::numeric_limits<double>::lowest());
    for (const auto &point : cloud->points) {
        min_point->x() = std::min(min_point->x(), static_cast<double>(point.x));
        min_point->y() = std::min(min_point->y(), static_cast<double>(point.y));
        min_point->z() = std::min(min_point->z(), static_cast<double>(point.z));
        max_point->x() = std::max(max_point->x(), static_cast<double>(point.x));
        max_point->y() = std::max(max_point->y(), static_cast<double>(point.y));
        max_point->z() = std::max(max_point->z(), static_cast<double>(point.z));
    }
    return true;
}
...
pcl_cloudslam::pcl_cloudslam():rclcpp::Node("pcl_cloudslam_node"),viewer(new pcl::visualization::PCLVisualizer("Pointcloud"))
{
    ...
    declare_parameter<double>("corner_patch_ratio", 0.15);
    declare_parameter<int>("platform_candidate_min_points", 80);
    ...
    declare_parameter<int>("workpiece_min_roi_points", 500);
...
bool pcl_cloudslam::refine_roi_by_platform_and_normal()
{
    double platform_plane_threshold;
    double roi_height_min;
    double roi_height_max;
    int roi_normal_k;
    double roi_normal_dot_min;
    double roi_cluster_tolerance;
    int roi_min_cluster_size;
    int roi_max_cluster_size;
    double corner_patch_ratio;
    int platform_candidate_min_points;
    get_parameter("platform_plane_threshold", platform_plane_threshold);
    get_parameter("roi_height_min", roi_height_min);
    get_parameter("roi_height_max", roi_height_max);
    get_parameter("roi_normal_k", roi_normal_k);
    get_parameter("roi_normal_dot_min", roi_normal_dot_min);
    get_parameter("roi_cluster_tolerance", roi_cluster_tolerance);
    get_parameter("roi_min_cluster_size", roi_min_cluster_size);
    get_parameter("roi_max_cluster_size", roi_max_cluster_size);
    get_parameter("corner_patch_ratio", corner_patch_ratio);
    get_parameter("platform_candidate_min_points", platform_candidate_min_points);

    platform_plane_threshold = std::max(1e-4, platform_plane_threshold);
    roi_height_min = std::max(0.0, roi_height_min);
    roi_height_max = std::max(roi_height_min + 1e-4, roi_height_max);
    roi_cluster_tolerance = std::max(1e-4, roi_cluster_tolerance);
    platform_candidate_min_points = std::max(3, platform_candidate_min_points);

    PointCloudXYZ::Ptr cloud_roi(new PointCloudXYZ);
    if (pcl::io::loadPCDFile(file_path + "pcl_roi.pcd", *cloud_roi) != 0 || cloud_roi->points.empty()) {
        RCLCPP_ERROR(get_logger(), "Failed to load non-empty coarse ROI: %s", (file_path + "pcl_roi.pcd").c_str());
        return false;
    }
    RCLCPP_INFO(get_logger(), "Concave ROI refine: coarse ROI points=%zu", cloud_roi->points.size());

    PointCloudXYZ::Ptr platform_candidates = extract_corner_candidates_xy(cloud_roi, corner_patch_ratio);
    pcl::io::savePCDFileASCII(file_path + "pcl_roi_platform_candidates.pcd", *platform_candidates);
    RCLCPP_INFO(
        get_logger(),
        "Concave ROI refine: corner platform candidate points=%zu, patch_ratio=%.3f",
        platform_candidates->points.size(),
        corner_patch_ratio
    );

    PlaneFitResult plane_fit;
    std::string plane_source = "corner_patches";
    const int candidate_min_inliers = std::max(3, platform_candidate_min_points / 2);
    bool plane_ok =
        static_cast<int>(platform_candidates->points.size()) >= platform_candidate_min_points &&
        fit_plane_ransac(platform_candidates, platform_plane_threshold, candidate_min_inliers, &plane_fit);

    if (!plane_ok) {
        plane_source = "full_roi_fallback";
        RCLCPP_WARN(
            get_logger(),
            "Corner platform candidates insufficient or unstable; fallback to full ROI plane RANSAC."
        );
        const int full_roi_min_inliers = std::max(3, static_cast<int>(cloud_roi->points.size() * 0.05));
        plane_ok = fit_plane_ransac(cloud_roi, platform_plane_threshold, full_roi_min_inliers, &plane_fit);
    }

    if (!plane_ok) {
        RCLCPP_ERROR(get_logger(), "Failed to fit platform plane for concave ROI refinement.");
        return false;
    }

    orient_plane_toward_workpiece(cloud_roi, &plane_fit.coefficients);
    Eigen::Vector3d platform_normal = plane_fit.coefficients.head<3>();
    if (platform_normal.norm() < 1e-8) {
        RCLCPP_ERROR(get_logger(), "Fitted platform normal is invalid.");
        return false;
    }
    platform_normal.normalize();
    last_platform_normal_in_base_ = platform_normal;
    has_platform_normal_ = true;

    RCLCPP_INFO(
        get_logger(),
        "Platform plane source=%s, inliers=%d, normal=[%.4f, %.4f, %.4f], d=%.5f",
        plane_source.c_str(),
        plane_fit.inlier_count,
        platform_normal(0),
        platform_normal(1),
        platform_normal(2),
        plane_fit.coefficients(3)
    );

    PointCloudXYZ::Ptr cloud_no_platform = filter_by_platform_height(
        cloud_roi,
        plane_fit.coefficients,
        roi_height_min,
        roi_height_max
    );
    pcl::io::savePCDFileASCII(file_path + "pcl_roi_no_platform.pcd", *cloud_no_platform);
    if (cloud_no_platform->points.empty()) {
        RCLCPP_ERROR(
            get_logger(),
            "Concave ROI refine removed all points after height filtering: height=[%.4f, %.4f]",
            roi_height_min,
            roi_height_max
        );
        return false;
    }
    RCLCPP_INFO(get_logger(), "Concave ROI refine: no-platform points=%zu", cloud_no_platform->points.size());

    PointCloudXYZ::Ptr cloud_no_side = filter_by_normal_dot(
        cloud_no_platform,
        platform_normal,
        roi_normal_k,
        roi_normal_dot_min
    );
    pcl::io::savePCDFileASCII(file_path + "pcl_roi_no_side.pcd", *cloud_no_side);
    if (cloud_no_side->points.empty()) {
        RCLCPP_ERROR(
            get_logger(),
            "Concave ROI refine removed all points after normal filtering: normal_dot_min=%.3f",
            roi_normal_dot_min
        );
        return false;
    }
    RCLCPP_INFO(get_logger(), "Concave ROI refine: no-side points=%zu", cloud_no_side->points.size());

    size_t cluster_count = 0;
    PointCloudXYZ::Ptr cloud_refined = keep_largest_cluster(
        cloud_no_side,
        roi_cluster_tolerance,
        roi_min_cluster_size,
        roi_max_cluster_size,
        &cluster_count
    );
    if (cloud_refined->points.empty()) {
        RCLCPP_ERROR(
            get_logger(),
            "Concave ROI refine found no valid cluster: clusters=%zu, min_cluster_size=%d",
            cluster_count,
            roi_min_cluster_size
        );
        return false;
    ...
    pcl::io::savePCDFileASCII(file_path + "pcl_roi_refined.pcd", *cloud_refined);
    RCLCPP_INFO(
        get_logger(),
        "Concave ROI refine complete: clusters=%zu, refined points=%zu",
        cluster_count,
        cloud_refined->points.size()
    );
    return true;
}

bool pcl_cloudslam::fit_local_workpiece_frame_from_refined_roi()
{
    PointCloudXYZ::Ptr cloud_refined(new PointCloudXYZ);
    const std::string roi_path = file_path + "pcl_roi_refined.pcd";
    if (pcl::io::loadPCDFile(roi_path, *cloud_refined) != 0 || cloud_refined->points.empty()) {
        RCLCPP_ERROR(get_logger(), "Failed to load non-empty refined ROI: %s", roi_path.c_str());
        return false;
    }

    WorkpieceFrameParams params;
    get_parameter("workpiece_center_percentile_low", params.center_percentile_low);
    get_parameter("workpiece_center_percentile_high", params.center_percentile_high);
    get_parameter("workpiece_normal_fit_radius", params.normal_fit_radius);
    get_parameter("workpiece_normal_fit_k", params.normal_fit_k);
    get_parameter("workpiece_min_normal_fit_points", params.min_normal_fit_points);
    get_parameter("workpiece_min_roi_points", params.min_roi_points);
    get_parameter("workpiece_axis_projection_min_norm", params.axis_projection_min_norm);

    bool workpiece_use_platform_normal;
    get_parameter("workpiece_use_platform_normal", workpiece_use_platform_normal);
    Eigen::Vector3d preferred_z_axis = Eigen::Vector3d::UnitZ();
    if (workpiece_use_platform_normal && has_platform_normal_) {
        preferred_z_axis = last_platform_normal_in_base_;
    }

    WorkpieceFrameResult result;
    if (!fit_workpiece_frame(cloud_refined, params, preferred_z_axis, &result)) {
        RCLCPP_ERROR(
            get_logger(),
            "Failed to fit local workpiece frame. roi_points=%zu, min_roi_points=%d, normal_fit_radius=%.4f, normal_fit_k=%d, min_normal_points=%d",
            cloud_refined->points.size(),
            params.min_roi_points,
            params.normal_fit_radius,
            params.normal_fit_k,
            params.min_normal_fit_points
        );
        return false;
    }

    T_base_from_workpiece_ = result.T_base_from_workpiece;
    T_workpiece_from_base_ = result.T_workpiece_from_base;
    workpiece_origin_in_base_ = result.origin;
    workpiece_x_axis_in_base_ = result.x_axis;
    workpiece_y_axis_in_base_ = result.y_axis;
    workpiece_z_axis_in_base_ = result.z_axis;
    has_workpiece_frame_ = true;

    const std::string base_from_workpiece_path = file_path + "T_base_from_workpiece.txt";
    const std::string workpiece_from_base_path = file_path + "T_workpiece_from_base.txt";
    if (!save_matrix4d(base_from_workpiece_path, T_base_from_workpiece_) ||
        !save_matrix4d(workpiece_from_base_path, T_workpiece_from_base_))
    {
        RCLCPP_ERROR(get_logger(), "Failed to save workpiece transform matrices.");
        return false;
    }

    const std::string frame_info_path = file_path + "workpiece_frame_info.txt";
    std::ofstream info_file(frame_info_path, std::ios::trunc);
    if (!info_file.is_open()) {
        RCLCPP_ERROR(get_logger(), "Failed to save workpiece frame info: %s", frame_info_path.c_str());
        return false;
    }
    info_file << "origin " << result.origin.transpose() << "\n";
    info_file << "x_axis " << result.x_axis.transpose() << "\n";
    info_file << "y_axis " << result.y_axis.transpose() << "\n";
    info_file << "z_axis " << result.z_axis.transpose() << "\n";
    info_file << "det_R " << result.det_R << "\n";
    info_file << "dot_xy " << result.dot_xy << "\n";
    info_file << "dot_xz " << result.dot_xz << "\n";
    info_file << "dot_yz " << result.dot_yz << "\n";
    info_file << "roi_point_count " << result.roi_point_count << "\n";
    info_file << "normal_fit_point_count " << result.normal_fit_point_count << "\n";
    info_file << "preferred_z_axis " << preferred_z_axis.transpose() << "\n";
    info_file << "used_platform_normal " << (workpiece_use_platform_normal && has_platform_normal_ ? 1 : 0) << "\n";

    RCLCPP_INFO(
        get_logger(),
        "Workpiece frame fitted: origin=[%.5f, %.5f, %.5f], x=[%.4f, %.4f, %.4f], y=[%.4f, %.4f, %.4f], z=[%.4f, %.4f, %.4f], det=%.6f, normal_fit_points=%zu",
        result.origin(0), result.origin(1), result.origin(2),
        result.x_axis(0), result.x_axis(1), result.x_axis(2),
        result.y_axis(0), result.y_axis(1), result.y_axis(2),
        result.z_axis(0), result.z_axis(1), result.z_axis(2),
        result.det_R,
        result.normal_fit_point_count
    );
    RCLCPP_INFO(
        get_logger(),
        "Workpiece frame outputs: %s, %s, %s",
        base_from_workpiece_path.c_str(),
        workpiece_from_base_path.c_str(),
        frame_info_path.c_str()
    );
    return true;
}

bool pcl_cloudslam::transform_refined_roi_to_workpiece_frame()
{
    if (!has_workpiece_frame_) {
        RCLCPP_ERROR(get_logger(), "Workpiece frame is not available, cannot transform refined ROI.");
        return false;
    }

    PointCloudXYZ::Ptr cloud_refined(new PointCloudXYZ);
    const std::string input_path = file_path + "pcl_roi_refined.pcd";
    if (pcl::io::loadPCDFile(input_path, *cloud_refined) != 0 || cloud_refined->points.empty()) {
        RCLCPP_ERROR(get_logger(), "Failed to load non-empty refined ROI: %s", input_path.c_str());
        return false;
    }

    PointCloudXYZ::Ptr cloud_workpiece = transform_cloud_to_workpiece(
        cloud_refined,
        T_workpiece_from_base_
    );
    if (cloud_workpiece->points.empty()) {
        RCLCPP_ERROR(get_logger(), "Transformed workpiece ROI is empty.");
        return false;
    }

    const std::string output_path = file_path + "pcl_roi_refined_workpiece.pcd";
    pcl::io::savePCDFileASCII(output_path, *cloud_workpiece);

    Eigen::Vector3d min_point;
    Eigen::Vector3d max_point;
    if (compute_cloud_bounds(cloud_workpiece, &min_point, &max_point)) {
        RCLCPP_INFO(
            get_logger(),
            "Workpiece ROI transformed: points=%zu, bounds_x=[%.5f, %.5f], bounds_y=[%.5f, %.5f], bounds_z=[%.5f, %.5f]",
    ...
    RCLCPP_INFO(get_logger(), "Workpiece ROI output: %s", output_path.c_str());
    return true;
}
...
bool pcl_cloudslam::path_plan_concave()
{
    ...
    if (!has_workpiece_frame_) {
        const std::string transform_path = file_path + "T_base_from_workpiece.txt";
        if (!load_matrix4d(transform_path, &T_base_from_workpiece)) {
            RCLCPP_ERROR(get_logger(), "Failed to load transform matrix: %s", transform_path.c_str());
            return false;
        }
    }

    ConcavePathParams params;
    get_parameter("concave_probe_radial_length", params.probe_radial_length);
    get_parameter("concave_radial_step", params.radial_step);
    get_parameter("concave_boundary_margin", params.boundary_margin);
    get_parameter("concave_r_end", params.r_end);
    get_parameter("concave_scan_point_spacing", params.scan_point_spacing);
    get_parameter("concave_delta_phi_min_deg", params.delta_phi_min_deg);
    get_parameter("concave_delta_phi_max_deg", params.delta_phi_max_deg);
    get_parameter("concave_r_fit", params.r_fit);
    get_parameter("concave_phi_fit_deg", params.phi_fit_deg);
    get_parameter("concave_min_fit_points", params.min_fit_points);
    get_parameter("concave_normal_fit_radius", params.normal_fit_radius);
    get_parameter("concave_min_normal_fit_points", params.min_normal_fit_points);
    ...
    params.tool_z_negative_normal = (tool_z_align != "positive_normal");
    ...
    const Eigen::Matrix3d tool_mount_rotation = rpy_to_rotation_matrix(
        tool_mount_rx,
        tool_mount_ry,
        tool_mount_rz
    );
    ...
    if (!generate_concave_path(
            cloud_workpiece,
            T_base_from_workpiece,
            params,
            probe_offset,
            tool_mount_rotation,
            &result))
    {
        RCLCPP_ERROR(
            get_logger(),
            "Concave path planning failed. roi_points=%zu, valid_points=%zu, candidates=%zu, skipped(theta=%zu, normal=%zu, orientation=%zu)",
            cloud_workpiece->points.size(),
    ...
    const std::string path_base_path = plan_path + "path_map_concave.txt";
    std::ofstream path_base_file(path_base_path, std::ios::trunc);
    if (!path_base_file.is_open()) {
        RCLCPP_ERROR(get_logger(), "Failed to open concave base path file: %s", path_base_path.c_str());
        return false;
    }
    for (const auto &pose : result.base_poses) {
        path_base_file << pose(0) << " " << pose(1) << " " << pose(2) << " "
                       << pose(3) << " " << pose(4) << " " << pose(5) << "\n";
    }

    const std::string path_workpiece_path = plan_path + "path_map_concave_workpiece.txt";
    std::ofstream path_workpiece_file(path_workpiece_path, std::ios::trunc);
    if (!path_workpiece_file.is_open()) {
        RCLCPP_ERROR(get_logger(), "Failed to open concave workpiece path file: %s", path_workpiece_path.c_str());
        return false;
    }
    for (const auto &point : result.workpiece_points) {
        path_workpiece_file
            << point.surface_point(0) << " " << point.surface_point(1) << " " << point.surface_point(2) << " "
            << point.normal(0) << " " << point.normal(1) << " " << point.normal(2) << " "
            << point.r << " " << point.theta << " " << point.phi << " "
            << point.layer_id << " " << point.point_id << "\n";
    }

    const std::string debug_path = plan_path + "path_debug_concave.txt";
    std::ofstream debug_file(debug_path, std::ios::trunc);
    if (!debug_file.is_open()) {
        RCLCPP_ERROR(get_logger(), "Failed to open concave debug path file: %s", debug_path.c_str());
        return false;
    }
    debug_file << "r_min " << result.r_min << "\n";
    debug_file << "r_max " << result.r_max << "\n";
    debug_file << "r_95 " << result.r_95 << "\n";
    debug_file << "r_end " << params.r_end << "\n";
    debug_file << "layer_count " << result.layer_count << "\n";
    debug_file << "total_candidate_points " << result.total_candidate_points << "\n";
    debug_file << "valid_path_points " << result.valid_path_points << "\n";
    debug_file << "skipped_theta_fit_points " << result.skipped_theta_fit_points << "\n";
    debug_file << "skipped_normal_fit_points " << result.skipped_normal_fit_points << "\n";
    debug_file << "skipped_orientation_points " << result.skipped_orientation_points << "\n";

    RCLCPP_INFO(
        get_logger(),
        "Concave path planned: layers=%zu, candidates=%zu, valid=%zu, skipped(theta=%zu, normal=%zu, orientation=%zu), r95=%.5f",
        result.layer_count,
        result.total_candidate_points,
        result.valid_path_points,
        result.skipped_theta_fit_points,
    ...
    RCLCPP_INFO(
        get_logger(),
        "Concave path outputs: %s, %s, %s",
        path_base_path.c_str(),
        path_workpiece_path.c_str(),
        debug_path.c_str()
    );
    return true;
}
...
void pcl_cloudslam::roi_range()
{
    ...
    std::string planning_mode;
    get_parameter("planning_mode", planning_mode);
    const std::string roi_file = pointcloudslam_cpp::roi_input_file_for_mode(planning_mode);
    if (pcl::io::loadPCDFile(file_path + roi_file, *cloud_roi) != 0 || cloud_roi->points.empty()) {
        RCLCPP_ERROR(get_logger(), "Failed to load non-empty ROI file for mode '%s': %s", planning_mode.c_str(), (file_path + roi_file).c_str());
        roi_range_x(0) = 1.0;
        roi_range_x(1) = 0.0;
        roi_range_y(0) = 1.0;
        roi_range_y(1) = 0.0;
