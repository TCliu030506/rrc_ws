...
public:
    PclSub(std::string name)
        : Node(name)
    {
        sub_novel = this->create_subscription<sensor_msgs::msg::PointCloud2>("/camera/camera/depth/color/points", 10, std::bind(&PclSub::topic_callback, this, std::placeholders::_1));
    }
...
void PclSub::topic_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
