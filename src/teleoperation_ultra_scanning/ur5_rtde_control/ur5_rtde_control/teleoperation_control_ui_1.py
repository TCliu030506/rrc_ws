...
class Subscriber(Node):
    ...
    def __init__(self):
        ...
        # 用于“仅映射主端绕自身y轴旋转”的增量累计（避免角轴在 pi 附近跳变）
        self._last_master_rot = None  # scipy Rotation
        self._cum_master_y = 0.0

        # 位置、姿态映射比例（可根据实际手感自行调整）
        # # 平动
        # self._pos_scale = [0.05, 1.5, 0.05]      # X、Y、Z 比例
        # self._rot_scale = [0.0, 0.0, 0.0]      # RX、RY、RZ 比例
        # 旋转
        self._pos_scale = [0.0, 0.0, 0.0]      # X、Y、Z 比例
        self._rot_scale = [0.000, 0.2, 0.000]      # RX、RY、RZ 比例
    ...
    def _publish_target_pose_loop(self):
        ...
        while rclpy.ok() and self._running:
            ...
            if master_pose is not None:
                ...
                # 仅映射：主端绕自身y轴旋转 -> 从端TCP绕自身y轴旋转
                target_pose = self._map_master_to_slave_pose_rotatey(master_pose)
                # target_pose = self._map_master_to_slave_pose(master_pose)
                # target_pose = self._map_master_to_slave_pose_xyz(master_pose)
    ...
    def _map_master_to_slave_pose_xyz(self, master_pose):
        """
        基于“增量式映射”的主从位姿映射函数。
    ...
    def _map_master_to_slave_pose_rotatey(self, master_pose):
        # 如果收到“开启遥操作”（3），则在第一次进入时设置初始位姿并切到 10 状态
        if self._control_signal == 3:
            self._initial_pose = self.ur.get_tcp_pose()
            self.get_logger().info('Control started: Initial pose set.')
            self._ref_master_pose = None  # 重置主端参考位姿
            self._last_master_rot = None
            self._cum_master_y = 0.0
            # 更新为 10 表示“遥操作进行中”
            self._control_signal = 10
        ...
        # 首帧：建立主端旋转累计的起点（这里不能直接用 master_pose[4] 做差，
        # 因为 rx,ry,rz 是角轴/旋转向量分量，分量差不等价于“绕y轴”的旋转差）
        master_rot = R.from_rotvec(np.array(master_pose[3:6], dtype=float))
        if self._last_master_rot is None:
            self._last_master_rot = master_rot
            return self._initial_pose

        # 计算主端相对上一帧的“增量旋转”
        delta_rot = self._last_master_rot.inv() * master_rot
        self._last_master_rot = master_rot

        # 仅提取“绕主端自身y轴”的 twist 分量，并累计（连续且不受角轴跳变影响）
        step_y = self._twist_angle_about_axis(delta_rot, np.array([0.0, 1.0, 0.0]))
        self._cum_master_y += step_y

        # 应用比例系数：只用 rot_scale 的 y
        _, sry, _ = self._rot_scale
        theta_slave = self._cum_master_y * sry

        # 叠加到从端初始位姿：从端TCP绕自身y轴旋转 theta_slave
        target_pose = self.rotate_about_tcp_y(self._initial_pose, theta_slave)

        return target_pose

    def rotate_about_tcp_y(self, current_pose, delta_theta):
        # current_pose: [x, y, z, rx, ry, rz]
        # delta_theta: 需要绕TCP自身y轴旋转的角度（弧度）

        # 从端姿态是 UR 的旋转向量（角轴），可直接用 rotvec <-> Rotation 互转
        rot_current = R.from_rotvec(np.array(current_pose[3:6], dtype=float))

        # 右乘：表示在 TCP（自身）坐标系下追加一个绕 y 轴的增量旋转
        rot_delta = R.from_rotvec([0.0, float(delta_theta), 0.0])
        rot_new = rot_current * rot_delta
        new_rvec = rot_new.as_rotvec()

        target_pose = list(current_pose)
        target_pose[3:6] = new_rvec.tolist()
        return target_pose

    def _twist_angle_about_axis(self, rot: R, axis: np.ndarray) -> float:
        """从 rot 中提取绕给定 axis 的 twist（有符号角度，单位 rad）。

        rot: scipy.spatial.transform.Rotation
        axis: 3D 单位轴（在 rot 所在的坐标系里表达）
        """
        ...
        # scipy 的四元数顺序为 [x, y, z, w]
        q = rot.as_quat()
        v = q[:3]
        w = q[3]

        # 将四元数的向量部投影到 axis 上，得到 twist 四元数
        v_proj = axis * float(np.dot(v, axis))
        qt = np.array([v_proj[0], v_proj[1], v_proj[2], w], dtype=float)
        qt_n = np.linalg.norm(qt)
        if qt_n < 1e-12:
            return 0.0
        qt /= qt_n

        # 对于纯 twist: v = axis * sin(theta/2), w = cos(theta/2)
        sin_half = float(np.dot(qt[:3], axis))
        cos_half = float(qt[3])
